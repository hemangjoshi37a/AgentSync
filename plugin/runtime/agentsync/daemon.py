...
class Daemon:
    def __init__(self, cfg: C.Config) -> None:
        self.cfg = cfg
        self.priv = None  # Curve25519 key, loaded lazily only when the relay is used
    ...
    async def _already_running(self, path: str) -> bool:
        """True if a live daemon already owns the socket (avoid double-start races)."""
        if not os.path.exists(path):
            return False
        try:
            _, w = await asyncio.open_unix_connection(path)
            w.close()
            return True
        except OSError:
            return False  # stale socket from a crashed daemon
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            pass
        # 16 MB line limit: agents routinely exchange large code/context blocks;
        # asyncio's 64 KB default would truncate-and-crash those connections.
        server = await asyncio.start_unix_server(self._on_client, path=path, limit=16 * 1024 * 1024)
        os.chmod(path, 0o600)
    ...
    async def _on_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            pass
        except Exception as exc:  # noqa: BLE001 - one bad client must not crash the handler
            log.warning("client connection error: %s", exc)
        finally:
    ...
    async def _relay_loop(self) -> None:
        try:
            ...
            log.warning("websockets not installed — remote/relay disabled; running local-only")
            return
        try:
            if self.priv is None:
                self.priv = C.load_or_create_key()
        except Exception as exc:  # noqa: BLE001 - PyNaCl missing or key load error
            log.warning("E2E key unavailable (%s) — remote/relay disabled; running local-only", exc)
            return
        backoff = 1
        while True:
            try:
