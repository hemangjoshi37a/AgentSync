...
def load_or_create() -> Config:
    """Load (or first-time create) this node's config (no key).

    The Curve25519 key is loaded separately and lazily via load_or_create_key()
    so the local-only daemon can run without PyNaCl installed.
    """
    ensure_dir()
...
def load_or_create_key():
    """Load or create this node's Curve25519 private key.

    Requires PyNaCl — but only the remote/relay path needs it, so this is called
    lazily by the daemon's relay loop, never at local-hub startup.
    """
    ensure_dir()
    if KEY_FILE.exists():
        return crypto.load_private_key(KEY_FILE.read_text().strip())
